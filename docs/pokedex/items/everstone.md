# Everstone

!!! info "Item"

	<div style="display: flex; align-items: flex-start; gap: 15px;">
		<img src="https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/items/everstone.png" alt="Everstone" style="border: 1px solid; border-radius: 4px; padding: 4px; align-self: center; min-height: 60px; min-width: 60px;" />
		<div>
			<span markdown>**Category:** Training</span>
			<br/>
			<span markdown>**Flavor Text:** An item to be held by a Pokémon. The Pokémon holding this peculiar stone is prevented from evolving.</span>
		</div>
	</div>

## :material-information: Effect

!!! tip "Quick Summary"

    Held: Prevents level-based evolution from occuring.

## :material-tag: Attributes

<div class="grid cards" markdown>

- **:material-shape: Category**

	---

	Training

- **:material-currency-usd: Cost**

	---

	₽3,000

- **:material-fire: Fling Power**

	---

	30

</div>

## :material-pokeball: Wild Pokémon Encounters

The following Pokémon may hold this item when encountered in the wild:

| Pokémon | Platinum | Diamond | Pearl | Heartgold | Soulsilver |
|:--------|:--------:|:-------:|:-----:|:---------:|:----------:|
| [**#074 Geodude**](../pokemon/geodude.md) | — | — | — | — | — |
| [**#075 Graveler**](../pokemon/graveler.md) | — | — | — | — | — |
| [**#076 Golem**](../pokemon/golem.md) | — | — | — | — | — |
