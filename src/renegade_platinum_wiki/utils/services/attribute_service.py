...
from renegade_platinum_wiki.utils.text.text_util import name_to_id, parse_pokemon_forme
...
class AttributeService:
    ...
    def _update_ability(pokemon_id: str, pokemon_data: Pokemon, value: str) -> bool:
        ...
        if len(abilities) not in [2, 3]:
            logger.warning(
                f"Invalid ability format (expected 2 or 3 abilities): {value}"
            )
            return False
        ...
        for i, ability in enumerate(abilities):
            ...
            new_abilities.append(
                {"name": ability, "is_hidden": is_hidden, "slot": i + 1}
            )
